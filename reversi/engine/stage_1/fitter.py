...
def play_human_game(board_start,color):			
	...
	for move in range(80):
		...
		if len(moves_villain) > 0:
			...
			attempt_to_make_move_modify(int(input()), board, -color, neighbors, False)#random.choice(moves_villain)
...
try:
	boards_ever_generated = pickle.load(open('boards_ever_generated.p', "rb") )	
except:
	print('current_node loading error')
	pass
master_game_record = []	
...
while True:
	while time.clock() - t0 < 1*60*60:
		t2 = time.clock()
		while time.clock() - t2 < time_to_fit:
			killer_move_table = [list(range(64)) for stages in range(64)]
			hash_table = [{} for i in range(64)]
			t3 = time.clock()
			while time.clock() - t3 < 0.01:
				current_node.choose()
			boards_ever_generated += 1
				

		t1 = time.clock()
		alpha = [0.01, 0.01, 0.01, 0.01, 0.01, 0.005, 0.005, 0.005, 0.001, 0.001]
		print(len(global_training_board_list[0]))
		how_many_boards_should_there_be = int(boards_ever_generated / 2 + 5)
		for global_training_board_list_concrete in global_training_board_list:
			empties = global_training_board_list_concrete[0][0].count(0)
			stage = (empties-3)//5
			slice_size = len(global_training_board_list_concrete)
			
			while len(global_training_board_list_concrete) > how_many_boards_should_there_be:
				global_training_board_list_concrete.pop(0)
				
			deltas_training = []
			for board, board_features, result in global_training_board_list_concrete:
				prediction  = board_evaluation_table(board, board_features, empties, 0)
				deltas_training.append( (prediction - result ) )
				
			for global_training_board, delta in zip(global_training_board_list_concrete, deltas_training):
				for feature_index, dict in zip(global_training_board[1], dicts):
					dict[feature_index][stage] +=  -alpha[stage]*delta/slice_size	
		time_to_fit = time.clock() - t1
		
		
	latest_dictionary_number += 1	
	latest_dictionary_filename ='dicts_final_'+  str(latest_dictionary_number)+'.p'
	
	pickle.dump(dicts, open(latest_dictionary_filename, "wb") )
